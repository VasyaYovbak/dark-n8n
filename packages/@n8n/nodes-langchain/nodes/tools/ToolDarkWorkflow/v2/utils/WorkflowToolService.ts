...
export class WorkflowToolService {
	...
	// Store parent context with enhanced methods (sendChunk, etc.)
	private parentContext: ISupplyDataFunctions | null = null;
	...
	async createTool({
		...
		// Store the parent context with enhanced methods (sendChunk, etc.) for later use
		...
		const toolHandler = async (
			...
			for (let tryIndex = 0; tryIndex < maxTries; tryIndex++) {
				...
				let context = this.baseContext.cloneWith({
					runIndex: localRunIndex,
					inputData: [[{ json: { query } }]],
				});

				// After cloning, we need to restore the enhanced methods from parentContext
				// because cloneWith() creates a new instance without them
				context = this.enhanceContextWithParentMethods(context);
	...
	/**
	 * Enhances any context with parent context methods (sendChunk, sendResponse, etc.)
	 * This is used both for cloned contexts and sub-workflow contexts
	 */
	private enhanceContextWithParentMethods(context: ISupplyDataFunctions): ISupplyDataFunctions {
		// If we don't have a parent context with enhanced methods, return original context
		if (!this.parentContext) {
			return context;
		}

		const parentCtx = this.parentContext as any;

		// Check if parent context has the methods we want to propagate
		const hasEnhancedMethods =
			typeof parentCtx.sendChunk === 'function' ||
			typeof parentCtx.sendResponse === 'function' ||
			typeof parentCtx.sendMessageToUI === 'function';
		...
		// Bind parent methods to the context
		if (typeof parentCtx.sendChunk === 'function') {
			contextAny.sendChunk = parentCtx.sendChunk.bind(parentCtx);
		}

		if (typeof parentCtx.sendResponse === 'function') {
			contextAny.sendResponse = parentCtx.sendResponse.bind(parentCtx);
		}

		if (typeof parentCtx.sendMessageToUI === 'function') {
			contextAny.sendMessageToUI = parentCtx.sendMessageToUI.bind(parentCtx);
		}

		if (typeof parentCtx.putExecutionToWait === 'function') {
			contextAny.putExecutionToWait = parentCtx.putExecutionToWait.bind(parentCtx);
		}

		if (typeof parentCtx.isStreaming === 'function') {
			contextAny.isStreaming = parentCtx.isStreaming.bind(parentCtx);
		}

		// Propagate other useful methods
		if (typeof parentCtx.getExecutionDataById === 'function') {
			contextAny.getExecutionDataById = parentCtx.getExecutionDataById.bind(parentCtx);
		}

		if (typeof parentCtx.addExecutionHints === 'function') {
			contextAny.addExecutionHints = parentCtx.addExecutionHints.bind(parentCtx);
		}
	...
	/**
	 * Enhances the context to pass parent context methods to sub-workflow
	 * This allows methods like sendChunk to be available in the sub-workflow execution
	 */
	private enhanceContextForSubWorkflow(context: ISupplyDataFunctions): ISupplyDataFunctions {
		// If we don't have a parent context with enhanced methods, return original context
		if (!this.parentContext) {
			return context;
		}
		...
		// Use the shared method to enhance context
		const enhancedContext = this.enhanceContextWithParentMethods(context);
	...
	private async executeSubWorkflow(
		...
		try {
			// If we have a parent context with enhanced methods (like sendChunk),
			// we need to make them available in the sub-workflow execution
			const enhancedContext = this.enhanceContextForSubWorkflow(context);

			// CRITICAL: We need to intercept additionalData.executeWorkflow to pass our methods
			// into the new sub-workflow execution context
			const originalExecuteWorkflow = (enhancedContext as any).additionalData.executeWorkflow;
			const parentMethods = this.parentContext as any;

			// Wrap executeWorkflow to inject our methods into sub-workflow's additionalData
			(enhancedContext as any).additionalData.executeWorkflow = async function (
				workflowInfo: any,
				additionalData: any,
				options: any,
			) {
				// Create a wrapped additionalData that will propagate our methods
				const wrappedAdditionalData = {
					...
					_parentMethods: parentMethods
						? {
								...
								// sendMessageToUI needs special handling - we need to ensure it always uses parent's sendDataToUI
								// Create a wrapper that will always call parent's sendDataToUI regardless of context binding
			...
			receivedData = await enhancedContext.executeWorkflow(
				workflowInfo,
				items,
				runManager?.getChild(),
				{
			...
			// Restore original executeWorkflow
			(enhancedContext as any).additionalData.executeWorkflow = originalExecuteWorkflow;
